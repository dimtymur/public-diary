...
if (!csrf_check($csrf_key))
    redirect($dimport["auth/signin_page.phtml"]["redirect"]."&error=csrf-error");

if (!isset($_POST["submit"]))
    redirect($dimport["auth/signin_page.phtml"]["redirect"]."&error=submit-error");

require_once $dimport["user/user_funcs.php"]["path"];

if (!$uname_validate($_POST["username"]))
    redirect($dimport["auth/signin_page.phtml"]["redirect"]."&error=invalid-username");
$username = $_POST["username"];

if (!$email_validate($_POST["email"]))
    redirect($dimport["auth/signin_page.phtml"]["redirect"]."&error=invalid-email");
$email = $_POST["email"];

if (!$passwd_validate($_POST["password"], $_POST["password-conf"]))
    redirect($dimport["auth/signin_page.phtml"]["redirect"]."&error=invalid-passwd");
...
if (empty($user))
    redirect($dimport["auth/signin_page.phtml"]["redirect"]."&error=invalid-signin");
$user = $user[0];
